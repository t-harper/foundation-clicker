...
import { unlockTradeRoute, sendShip } from '@desktop/api';
import { Button } from '@desktop/components/common';
import { formatNumber, formatDuration, formatResource } from '@desktop/utils/format';

export function MobileTradeRouteManager() {
      ...
      {tradeRoutes.map((route) => {
        ...
        return (
          ...
          >
            ...
            <div className="px-4 pt-3 pb-2">
              <div className="flex items-center justify-between gap-2 mb-1">
                <div className="flex items-center gap-2 min-w-0">
                  ...
                  {route.isUnlocked ? (
                    <span className="shrink-0 px-1.5 py-0.5 rounded text-[10px] font-semibold bg-green-500/15 text-green-400">
                      Unlocked
                    </span>
                  ) : (
                    <span className="shrink-0 px-1.5 py-0.5 rounded text-[10px] font-semibold bg-red-500/15 text-red-400">
                      Locked
                    </span>
                  )}
                </div>
              ...
              <p className="text-[11px] text-[var(--era-text)]/50 leading-relaxed">
                {def.description}
              </p>
            </div>

            {/* Duration + ship requirement */}
            <div className="flex flex-wrap gap-x-4 gap-y-1 px-4 pb-2 text-xs">
            ...
            {/* Unlock button or send ship controls */}
            <div className="px-4 pb-3">
              {!route.isUnlocked ? (
                <div className="space-y-2">
                  {/* Unlock cost */}
                  <div className="flex flex-wrap gap-x-2 gap-y-0.5">
                    <span className="text-xs text-[var(--era-text)]/40">Cost:</span>
                  ...
                  <button
                    type="button"
                    disabled={!canAffordUnlock || isLoading}
                    onClick={() => handleUnlock(route.routeKey)}
                    className="w-full h-10 rounded-md text-xs font-semibold bg-[var(--era-accent)]/15 border border-[var(--era-accent)]/40 text-[var(--era-accent)] active:bg-[var(--era-accent)]/25 disabled:opacity-40 disabled:active:bg-[var(--era-accent)]/15 transition-colors touch-action-manipulation"
                  >
                    {isLoading ? 'Unlocking...' : 'Unlock Route'}
                  </button>
                </div>
              ) : (
